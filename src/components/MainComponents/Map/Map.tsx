...
export default function Map({
  ...
  setActiveLocationHandler: Function;
}) {
  ...
  const mapData: Function = function () {
    ...
    assets.forEach((locationData) => {
      ...
      if (Number(locationData.avg) < 0.45) {
        ...
        0.45 <= Number(locationData.avg) &&
        Number(locationData.avg) < 0.5
      ) {
        icon = Leaflet.icon({
        ...
        0.5 <= Number(locationData.avg) &&
        Number(locationData.avg) < 0.55
      ) {
        icon = Leaflet.icon({
      ...
      const location: number[] = [
        ...
        Number(locationData.location[1]),
      ];
      data.push(
        <Marker
          eventHandlers={{ click: (e) => setActiveLocationHandler(location) }}
          key={locationData.location.toString()}
          position={[
            Number(locationData.location[0]),
            Number(locationData.location[1]),
          ]}
