      ...
      for (const data of assets) {
        answer.push.apply(
          ...
          data.data.map((row) => {
            return {
              year: row.year,
              location: row.location.toLocaleString(),
              id: row.id,
              name: row.name,
              category: row.category,
              riskRating: row.riskRating,
              riskFactor: JSON.stringify(row.riskFactor),
            };
          })
        );
      }
    ...
    <main className="mt-10">
      ...
      >
        ...
        <Column
          ...
          dataType="numeric"
          filter
          filterPlaceholder="Search by risk rating"
          sortable
        />
        <Column
          field="riskFactor"
          header="Risk Factor"
          filter
          filterPlaceholder="Search by risk factor"
          sortable
        />
        <Column
          field="year"
